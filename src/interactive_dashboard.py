...
def check_db_tables_exist():
    """Ensure required tables exist in remote database; do not modify them."""
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("""SELECT table_name FROM information_schema.tables
                      WHERE table_schema = 'public'
                      AND table_name IN ('candidates','machine_mentions')""")
    existing = {row[0] for row in cursor.fetchall()}
    missing = {'candidates', 'machine_mentions'} - existing
    cursor.close()
    conn.close()
    if missing:
        logger.error(f"Missing required tables in database: {', '.join(sorted(missing))}")
        raise SystemExit(1)
...
if __name__ == "__main__":
    ...
    # Do not modify or reset the remote database from the dashboard runner.
    # Require the tables to exist and error out if they are missing.
    check_db_tables_exist()
